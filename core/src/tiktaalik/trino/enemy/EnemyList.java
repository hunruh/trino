 ...
 */
public class EnemyList implements Iterable<EnemyModel> {
...
//    private TextureRegion enemyTexture;
     ...
     */
    public EnemyList(int size, Vector2 scale, TextureRegion enemyTexture,PooledList<Obstacle> objects ) {
        enemies = new EnemyModel[size];
        float dwidth = enemyTexture.getRegionWidth() / scale.x;
        float dheight = enemyTexture.getRegionHeight() / scale.y;
        for (int ii = 0; ii < size; ii++) {
            enemies[ii] = new EnemyModel(28.25f, 5.0f,dwidth,dheight);
...
//        }
    ...
    public void addObjects(PooledList<Obstacle> objects){
        for (EnemyModel e :this){
            objects.add(e);
        }
    }
     ...
     */
    public Iterator<EnemyModel> iterator() {
     ...
     */
    private class EnemyIterator implements Iterator<EnemyModel> {
